...
DLLEXP my_bool http_get_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
DLLEXP char *http_get(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error);
DLLEXP void http_get_deinit(UDF_INIT *initid);

DLLEXP my_bool http_post_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
DLLEXP char *http_post(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error);
DLLEXP void http_post_deinit(UDF_INIT *initid);

DLLEXP my_bool http_put_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
DLLEXP char *http_put(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error);
DLLEXP void http_put_deinit(UDF_INIT *initid);

DLLEXP my_bool http_delete_init(UDF_INIT *initid, UDF_ARGS *args, char *message);
DLLEXP char *http_delete(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error);
DLLEXP void http_delete_deinit(UDF_INIT *initid);
...
static size_t result_cb(void *ptr, size_t size, size_t nmemb, void *data) {
                ...
                __attribute__ ((unused)) char *error)
{
  ...
  if (curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "lib_mysqludf_udf/1.0");
...
my_bool http_post_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  ...
  if (args->arg_count != 2) {
    strncpy(message, "two arguments must be supplied: http_post('<url>','<data>').", MYSQL_ERRMSG_SIZE);
    return 1;
  }
  args->arg_type[0]= STRING_RESULT;
  initid->max_length= CURL_UDF_MAX_SIZE;
...
my_bool http_put_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  ...
  if (args->arg_count != 2) {
    strncpy(message, "two arguments must be supplied: http_put('<url>','<data>').", MYSQL_ERRMSG_SIZE);
    return 1;
  }
  args->arg_type[0]= STRING_RESULT;
  initid->max_length= CURL_UDF_MAX_SIZE;
...
my_bool http_delete_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  ...
  if (args->arg_count != 1) {
    strncpy(message, "one arguments must be supplied: http_delete('<url>').", MYSQL_ERRMSG_SIZE);
    return 1;
  }
  args->arg_type[0]= STRING_RESULT;
  initid->max_length= CURL_UDF_MAX_SIZE;
